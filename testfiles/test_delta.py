...
def run_delta_test():
    ...
    delta_array = np.linspace(-5e-6, 5e-6, 50)
    ...
    # Filter out any 'inf' errors so the graph doesn't break
    ...
    plt.plot(delta_array[valid_idx] * 1e6, error_results[valid_idx] / 1e12, 'b-', linewidth=3, label='Frequency Error')
    
    min_idx = np.argmin(error_results[valid_idx])
    actual_best_delta = delta_array[valid_idx][min_idx]
